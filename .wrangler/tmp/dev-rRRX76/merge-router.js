...
function rewriteSigmabloxLocation(value) {
  ...
  if (value.startsWith("//")) {
    ...
    if (SIGMABLOX_HOSTNAMES.has(url.hostname)) {
      const path = url.pathname.startsWith("/combine") ? url.pathname.slice("/combine".length) || "/" : url.pathname;
...
var merge_router_default = {
  async fetch(request) {
    ...
    return new HTMLRewriter().on("header", {
      element(element) {
...
// .wrangler/tmp/bundle-R8GxYO/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-R8GxYO/middleware-loader.entry.ts
