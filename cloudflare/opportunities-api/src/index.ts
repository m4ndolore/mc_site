...
//   "drone OR UAS"  and  "artificial intelligence OR autonomous"
...
app.get("/api/opportunities", async (c) => {
        ...
        let samTotal = 0;
          ...
          // Strategy: two targeted title searches scoped to DoD, covering drone/UAS and
          // AI/autonomous topics. Results are then post-filtered by matchesKeywords() on
          // the title so only genuinely relevant opportunities appear.
          if (sources.includes("sam")) {
                    const samApiKey = c.env.SAM_GOV_API_KEY;
                    if (samApiKey) {
                                // If the caller provides a keyword, use that; otherwise run the two
                      // default DoD-scoped searches.
                      ...
                      await Promise.all(
                                    samSearchTerms.map(async (kw) => {
                                                    ...
                                                    if (json) {
                                                                      // Post-filter: title must match a cUAS/AI keyword
                      ...
                      // Deduplicate by noticeId across the two searches
                      const seen = new Set<string>();
