...
function buildSamGovUrl(apiKey: string, keyword: string): string {
  ...
  const params = new URLSearchParams({
          ...
          limit: "100",
          offset: "0",
          // Filter to DoD-relevant active special notices, solicitations, sources sought
          ptype: "s,o,r",
  });
...
app.get("/api/opportunities", async (c) => {
      ...
      let samTotal = 0;
          ...
          if (sources.includes("sam")) {
                  ...
                  if (samApiKey) {
                            // Build keyword list from cUAS/AI terms plus any user-supplied keyword
                    const samKeywords = keyword
                              ? [keyword]
                                : ["drone", "UAS", "autonomous", "artificial intelligence", "counter-UAS"];

                    const samResults: unknown[] = [];

                    await Promise.all(
                                samKeywords.map(async (kw) => {
                                              ...
                                              if (json) {
                                                              const parsed = parseSamGovResponse(json).filter((item) => {
                                                                                ...
                                                                                return matchesKeywords(
                                                                                                    `${i.topicTitle} ${i.description} ${i.tags}`
                                                                                                  );
                                                              });
                    ...
                    // Deduplicate by topicCode (noticeId)
                    const seen = new Set<string>();
